...
func TestConstructor(t *testing.T) {
	...
	if !q2.IsEmpty() {
		t.Error("new MyQueue should be empty")
	}
}
...
func TestPushPopGet(t *testing.T) {
	...
	// Test Get on empty
	if _, err := q.Get(); err != ErrMyQueueEmpty {
		t.Errorf("expected ErrMyQueueEmpty on Get empty, got %v", err)
	}

	// Test Pop on empty
	if _, err := q.Pop(); err != ErrMyQueueEmpty {
		t.Errorf("expected ErrMyQueueEmpty on Pop empty, got %v", err)
	}
...
func TestSaveText(t *testing.T) {
	...
	// Success
	filename, cleanup := createTempFile(t, "MyQueue_text_*.txt")
	defer cleanup()
...
func TestLoadText(t *testing.T) {
	// Success Case
	{
		filename, cleanup := createTempFile(t, "good_MyQueue.txt")
		defer cleanup()
...
func TestSaveBinary(t *testing.T) {
	q := NewMyQueue[float64](5)
	q.Push(1.1)

	// Success
	filename, cleanup := createTempFile(t, "MyQueue_bin_*.gob")
	defer cleanup()
...
func TestLoadBinary(t *testing.T) {
	// Success
	{
		filename, cleanup := createTempFile(t, "MyQueue_good.gob")
		defer cleanup()

		// Save valid data first
		qOrig := NewMyQueue[string](2)
		qOrig.Push("Test")
		qOrig.SaveBinary(filename)

		qLoad := NewMyQueue[string](1)
