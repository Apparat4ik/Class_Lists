...
func captureOutputSt(f func()) string {
	old := os.Stdout
	r, w, _ := os.Pipe()
	os.Stdout = w
...
func TestNewMyStack(t *testing.T) {
	s := NewMyStack[int]()
	if s == nil {
		t.Fatal("NewMyStack returned nil")
	}
	if s.Size() != 0 {
...
func TestNewMyStackWithCapacity(t *testing.T) {
	// Успешное создание
	s, err := NewMyStackWithCapacity[string](10)
	if err != nil {
	...
	// Ошибка (capacity <= 0)
	_, err = NewMyStackWithCapacity[string](0)
	if err == nil {
	...
	_, err = NewMyStackWithCapacity[string](-5)
	if err == nil {
...
func TestPushAndPop(t *testing.T) {
	...
	_, err = s.Pop()
	if err == nil {
		t.Error("Expected error when popping from empty MyStack")
	}
	if err.Error() != "MyStack underflow: cannot pop from an empty MyStack" {
		t.Errorf("Incorrect error message: %v", err)
	}
}

func TestAutoResize(t *testing.T) {
	s, _ := NewMyStackWithCapacity[int](1)
...
func TestCloneSt(t *testing.T) {
	original := NewMyStack[int]()
	original.Push(1)
	original.Push(2)
	...
	if original.Size() != 2 {
		t.Error("Original MyStack modified after clone manipulation")
	}

	val, _ := original.Pop()
	if val != 2 {
		t.Error("Original MyStack content corrupted")
	}
}

func TestPrintSt(t *testing.T) {
...
func TestSaveTextSt(t *testing.T) {
	tmpDir := t.TempDir()
	goodFile := filepath.Join(tmpDir, "MyStack.txt")
...
func TestLoadTextSt(t *testing.T) {
	tmpDir := t.TempDir()
	...
	} else if !strings.Contains(err.Error(), "failed to read MyStack size") {
...
func TestSaveBinarySt(t *testing.T) {
	tmpDir := t.TempDir()
	goodFile := filepath.Join(tmpDir, "MyStack.bin")

	s := NewMyStack[float64]()
	s.Push(3.14)
...
func TestLoadBinarySt(t *testing.T) {
	tmpDir := t.TempDir()

	// Подготовка файла для успешной загрузки
	goodFile := filepath.Join(tmpDir, "MyStack.bin")
	sOrig := NewMyStack[float64]()
	sOrig.Push(1.23)
	sOrig.SaveBinary(goodFile)

	// Успешная загрузка
	sLoad := NewMyStack[float64]()
